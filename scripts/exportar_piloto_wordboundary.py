#!/usr/bin/env python3
"""Renderiza o piloto oficial S001-S006 WordBoundary via MediaRecorder."""
...
OUTPUT = RENDERS_DIR / "capital_oculto_pilot_s001_s006_v2_wordboundary.webm"
METADATA = RENDERS_DIR / "render_metadata_wordboundary.json"
BROWSERS = (
...
def browser() -> Path:
    for candidate in BROWSERS:
        if candidate.is_file():
            return candidate
    raise ExportError("Edge ou Chrome nao encontrado.")
...
def main() -> int:
    ...
    for required in (
        PILOT_DIR / "player_wordboundary.html",
        PILOT_DIR / "manifest" / "scene_manifest_wordboundary.json",
        PILOT_DIR / "audio" / "narration_wordboundary.wav",
    ):
        if not required.is_file():
            raise ExportError(f"Arquivo obrigatorio ausente: {required}")
    if OUTPUT.exists():
        raise ExportError(f"Render ja existe e nao sera sobrescrito: {OUTPUT}")
    RENDERS_DIR.mkdir(parents=True, exist_ok=True)
    State.done.clear()
    State.error = None
    State.headers = {}
    server = http.server.ThreadingHTTPServer(("127.0.0.1", 0), functools.partial(handler(OUTPUT), directory=str(PILOT_DIR)))
    thread = threading.Thread(target=server.serve_forever, daemon=True)
    thread.start()
    profile = Path(tempfile.mkdtemp(prefix="co-av-wordboundary-"))
    process = None
    try:
        command = [
            ...
            f"--user-data-dir={profile}", f"http://127.0.0.1:{server.server_address[1]}/player_wordboundary.html",
        ]
        ...
        if not State.done.wait(args.timeout):
            raise ExportError(f"Exportacao excedeu {args.timeout}s.")
        if State.error:
            raise ExportError(f"Erro no player: {State.error}")
        if not OUTPUT.is_file() or OUTPUT.stat().st_size < 100_000:
            raise ExportError("O navegador nao produziu um WebM valido.")
    finally:
        server.shutdown()
        server.server_close()
